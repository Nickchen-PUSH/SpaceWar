...
export class FreeCameraController extends Entity {
    ...
    update(delta: number) {
        ...
        // W/S - 前后 (沿着相机朝向)
        if (input.getKey("KeyW")) vec3.add(moveDir, moveDir, [0, 0, -1]);
        if (input.getKey("KeyS")) vec3.add(moveDir, moveDir, [0, 0, 1]);

        // A/D - 左右 (沿着相机右向量)
        if (input.getKey("KeyA")) vec3.add(moveDir, moveDir, [-1, 0, 0]);
        if (input.getKey("KeyD")) vec3.add(moveDir, moveDir, [1, 0, 0]);
